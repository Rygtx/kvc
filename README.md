...
**UnderVolter — EFI undervolting module (Ring-1)** — KVC now ships with `UnderVolter.dat`, an encrypted UEFI payload that deploys a custom EFI application directly to the EFI System Partition. On boot, the loader runs before the Windows kernel and applies Intel voltage/power-limit offsets via MSR writes at firmware privilege level (Ring-1 / SMM-adjacent). Deployment is handled entirely through `kvc undervolter deploy` without third-party tools: KVC locates the ESP by its GPT partition GUID (`C12A7328-F81F-11D2-BA4B-00A0C93EC93B`) using `FindFirstVolume` + `IOCTL_DISK_GET_PARTITION_INFO_EX` — no drive-letter assignment, no `mountvol`. Two deployment modes: **A** replaces `\EFI\BOOT\BOOTX64.EFI` (transparent, runs on every boot with the original backed up as `BOOTX64.efi.bak`); **B** copies files to `\EFI\UnderVolter\` for a manual UEFI boot entry.

**Plundervolt-class research capability** — Operating at Ring-1, the undervolting module enables systematic exploration of the Plundervolt attack surface (CVE-2019-11157) on pre-patched microcode or custom firmware. By dynamically adjusting the CPU core voltage offset mid-computation via `MSR 0x150` (Intel Voltage Control MSR), the module can induce controlled voltage glitches into cryptographic operations running in SGX enclaves or kernel context — allowing researchers to study fault-injection behaviour and validate hardware-level mitigations without physical probing equipment. Not applicable to systems with microcode `>=` `0x2F` (Coffee Lake) or `BIOS_OVRD_EN` disabled in ME firmware.

**Per-generation CPU configuration via `UnderVolter.ini`** — The module ships with a documented `UnderVolter.ini` covering Intel **2nd through 15th generation** Core processors: Sandy Bridge, Ivy Bridge, Haswell, Broadwell, Skylake, Kaby Lake, Coffee Lake (8th/9th gen), Comet Lake, Tiger Lake, Rocket Lake, Alder Lake, Raptor Lake, Meteor Lake, and Arrow Lake (Core Ultra 200S/HX). Each profile is identified by CPUID (family/model) and defines safe voltage offset ranges per domain (`IACORE`, `RING`, `ECORE`, `UNCORE`, `GTSLICE`, `GTUNSLICE`), IccMax limits, and power-limit values where applicable. All offsets include a 20% safety margin based on community-reported stable values. The framework selects the matching profile automatically at boot time via CPUID — no manual tuning required on supported platforms. **Lunar Lake (Core Ultra 200V)** is explicitly not supported: its embedded power delivery bypasses the traditional `MSR 0x150` OC Mailbox interface entirely. Full documentation, raw binaries, and EFI application source available at **[kvc.pl/repositories/undervolter](https://kvc.pl/repositories/undervolter)**. The `.dat` package is built with `KvcXor.exe` option 6 (`Loader.efi + UnderVolter.efi + UnderVolter.ini -> UnderVolter.dat`).
